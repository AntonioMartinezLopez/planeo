...
Verified against `auth/local/realm.json`: one realm per environment (e.g. realm `local`), and within it, one dedicated **group** and one dedicated **client** per tenant/organization (currently a single tenant, also named `local`). `modules/keycloak-org` formalizes this: one module call per organization, creating that org's group + client + roles + authorization config within the calling environment's realm. Onboarding a new org becomes one `module` block plus `tofu apply`, instead of hand-editing the JSON exports.
...
**`modules/keycloak-org`** — one call per tenant/organization within an environment's realm (Section 2): the client (confidential, service accounts on, direct grants on — matching today's `local` client), client roles (`Admin`, `Planner`, `User`, `uma_protection`), a group at `/<org_name>`, and an internal call to `keycloak-client-authz` for that client. Splitting identity (this module) from authorization schema (`keycloak-client-authz`) is a real conceptual boundary, not just organizing for its own sake — `keycloak-client-authz` could be reused against a non-org client later (e.g. a service client) without touching org-provisioning logic.
