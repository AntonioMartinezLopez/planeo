...
- **`EmailReceivedConsumerAdapter`**: no test at all, for now. A unit test was tried first — `processRecord` calls `llm.ExtractRequestFields`/`llm.ClassifyRequest` directly as free functions, not through an injectable port, so exercising it made live network calls to the real Mistral API and required `MISTRAL_API_KEY` in the process environment, not appropriate for a hermetic unit test — and was retracted. An integration-level rollback test was tried next (see below, also retracted). This adapter currently has no dedicated test coverage of its own; its correctness rests on the pieces it composes (the `Repository`, tested separately below; `request.Service`/`category.Service`, tested at the domain layer) being individually correct, plus the whole-branch final review.
...
- **`services/core`'s consumer-adapter rollback atomicity** — tried, then retracted. The intent (mirroring the outbox-architecture-cleanup plan's "`CreateOutboxEvent` failure rolls back `CreateMail`" integration test): force `UpdateRequest` to fail after a successful `CreateRequest` inside the adapter's transaction, then assert against real Postgres that no `Request` row persists and the inbox row is back to `pending`. The flaw found during implementation: the verification step used the same instance id that claimed the row — because of the `claimed_by` fast-reclaim mechanism, a row stuck at `processing` under that same instance id is indistinguishable from a genuinely `pending` one, so the test could not actually discriminate correct rollback behavior from the exact regression (`MarkFailed` called inside the already-aborted transaction, silently losing the attempts increment) it existed to catch. A fix was identified (use a different instance id for the verification call) but the test was removed rather than fixed, alongside the unit test above — revisit both together later.
