...
- No message-level retry/DLQ logic — on handler error, log and skip the offset commit. Nothing more. Note this is not a redelivery-with-delay mechanism: because Kafka commits are cumulative per partition, the skipped record is not actually redelivered once a later record in the same partition commits — it's a silent drop, not a delay.
