...
// UpsertRequest writes every column of req in a single statement. id is
// generated by Postgres (never taken from req) and reference_id/
// organization_id are the conflict target, so neither is touched on
// conflict — mirroring CreateRequest's no-op reference_id update.
func (c *Client) UpsertRequest(ctx context.Context, req request.Request) (int, error) {
	query := `
		INSERT INTO requests (text, name, subject, email, address, telephone, raw, closed, reference_id, organization_id, category_id)
		VALUES (@text, @name, @subject, @email, @address, @telephone, @raw, @closed, @referenceId, @organizationId, @categoryId)
		ON CONFLICT (organization_id, reference_id) WHERE reference_id <> '' DO UPDATE SET
			text = EXCLUDED.text,
			name = EXCLUDED.name,
	...
	err := q.QueryRow(ctx, query, args).Scan(&id)
	if err != nil {
		return 0, NewDatabaseError("error upserting request", err)
	}
