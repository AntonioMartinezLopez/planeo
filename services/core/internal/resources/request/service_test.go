...
func TestRequestService(t *testing.T) {
	...
	t.Run("GetRequests", func(t *testing.T) {
		...
		t.Run("returns requests when requests are fetched successfully", func(t *testing.T) {
			mockRequestRepository := mocks.NewMockRequestRepositoryInterface(t)
			mockRequestRepository.EXPECT().GetRequests(context.Background(), testOrganizationId, 0, 10, false, []int{}).Return([]models.Request{request}, nil)
			requestService := NewRequestService(mockRequestRepository)
		...
		t.Run("returns filtered requests when selectedCategories is provided", func(t *testing.T) {
			...
			mockRequestRepository.EXPECT().GetRequests(context.Background(), testOrganizationId, 0, 10, false, selectedCategories).Return([]models.Request{request}, nil)
			requestService := NewRequestService(mockRequestRepository)
